...
`filterx` is a command line tool to filter lines from stdin or files. It is inspired by [grep](https://www.gnu.org/software/grep/) and [awk](https://www.gnu.org/software/gawk/) and [bioawk](https://github.com/lh3/bioawk).
...
filterx can filter the csv file without a header. Use `col` to reference the column. The column index starts from 1. In the following example, `col(2)` refers to the second column in the csv file.

```shell
filterx csv example-without-header.csv -e 'col(2) > 25'
